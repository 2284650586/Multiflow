...
MulItem::MulItem(Multiflow_TYPE type, QString itemName, QGraphicsPixmapItem *parent) :
    QGraphicsPixmapItem(parent), itemName(itemName)
{
    mulType = type;
    QPixmap pixmap = QPixmap();
    if (type == Multiflow_TYPE::Well) {
        pixmap.load(":/image/Well.png");
    } else if (type == Multiflow_TYPE::Source) {
        pixmap.load(":/image/source.png");
    } else if (type == Multiflow_TYPE::Sink) {
        pixmap.load(":/image/sink.png");
    } else if (type == Multiflow_TYPE::Junction) {
        pixmap.load(":/image/junction.png");
    }
...
void MulItem::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    {
        if (mulType == Multiflow_TYPE::Source) {
            if (source == nullptr) {
                isNew = true;
                source = new MSource();
            }
        ...
        if (mulType == Multiflow_TYPE::Sink) {
            if (sink == nullptr) {
                isNew = true;
                sink = new MSink();
            }
        ...
        if (mulType == Multiflow_TYPE::Junction) {
            if (junction == nullptr) {
                isNew = true;
        ...
        if (mulType == Multiflow_TYPE::Well) {
