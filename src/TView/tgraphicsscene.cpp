...
void TGraphicsScene::setItemType(Multiflow_TYPE type)
{
    m_itemType = type;
}
...
bool TGraphicsScene::judgeConnect(MulItem *startItem, MulItem *endItem)
{
    ...
    if (startItem->MulType() == Multiflow_TYPE::Well) {
        if (startItem->getStartArrowNum() >= 1) flag = false;
        if (endItem->MulType() == Multiflow_TYPE::Well) flag = false;
        if (endItem->MulType() == Multiflow_TYPE::Source) flag = false;
    }

    if (startItem->MulType() == Multiflow_TYPE::Source) {
        if (startItem->getStartArrowNum() >= 1) flag = false;
        if (endItem->MulType() == Multiflow_TYPE::Well) flag = false;
        if (endItem->MulType() == Multiflow_TYPE::Source) flag = false;
    }

    if (startItem->MulType() == Multiflow_TYPE::Sink) {
        if (startItem->getStartArrowNum() >= 1) flag = false;
        if (endItem->MulType() == Multiflow_TYPE::Sink) flag = false;
    }

    if (startItem->MulType() == Multiflow_TYPE::Junction) flag = true;

    if (endItem->MulType() == Multiflow_TYPE::Well) {
        if (endItem->getEndArrowNum() >= 1) flag = false;
        if (startItem->MulType() == Multiflow_TYPE::Source) flag = false;
    }

    if (endItem->MulType() == Multiflow_TYPE::Source) {
        if (endItem->getEndArrowNum() >= 1) flag = false;
        if (startItem->MulType() == Multiflow_TYPE::Well) flag = false;
    }

    if (endItem->MulType() == Multiflow_TYPE::Sink) {
        if (endItem->getEndArrowNum() >= 1) flag = false;
    }

    if (endItem->MulType() == Multiflow_TYPE::Junction) flag = true;
...
{
    if (m_itemType == Multiflow_TYPE::Well) return QString("well");
    if (m_itemType == Multiflow_TYPE::Source) return QString("source");
    if (m_itemType == Multiflow_TYPE::Sink) return QString("sink");
    if (m_itemType == Multiflow_TYPE::Junction) return QString("junction");
    return QString("未命名");
}
