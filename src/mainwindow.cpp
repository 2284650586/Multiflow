...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent)
{
    setWindowIcon(QIcon(":/image/icon.jpeg"));
    resize(1300, 800);
    createActions();
    createToolBar();
    createMenu();
...
{
    newFileAction = new QAction(QIcon(":/image/file.png"), "新建文件", this);
    connect(newFileAction, &QAction::triggered, this, &MainWindow::createGraphicsView);

    deleteAction = new QAction(QIcon(":/image/delete.png"), "删除", this);
    deleteAction->setShortcut(QKeySequence::Delete);
    connect(deleteAction, &QAction::triggered, this, &MainWindow::deleteItem);

    newWellAction = new QAction(QIcon(":/image/Well.png"), "新建井", this);
    newWellAction->setData(Multiflow_TYPE::Well);
    connect(newWellAction, &QAction::triggered, this, &MainWindow::createMulItem);

    newSourceAction = new QAction(QIcon(":/image/source.png"), "新建源", this);
    newSourceAction->setData(Multiflow_TYPE::Source);
    connect(newSourceAction, &QAction::triggered, this, &MainWindow::createMulItem);

    newSinkAction = new QAction(QIcon(":/image/sink.png"), "新建sink", this);
    newSinkAction->setData(Multiflow_TYPE::Sink);
    connect(newSinkAction, &QAction::triggered, this, &MainWindow::createMulItem);

    newJunctionAction = new QAction(QIcon(":/image/junction.png"), "新建接合点", this);
    newJunctionAction->setData(Multiflow_TYPE::Junction);
...
{
    newFileMenu = menuBar()->addMenu("文件");
...
void MainWindow::createGraphicsView()
{
    ...
    tScene->setBackgroundBrush(QPixmap(":/image/background3.png"));
...
void MainWindow::createMulItem()
{
    ...
    scene->setItemType(Multiflow_TYPE(type));
