...
void MWellWindow::initModel() {
    ...
    QStringList headerCasing = {
        "Name", "Grade", "Density (g/cm3)", "Thermal conductivity (W/(m.k))", "To MD (m)", "ID (m)",
        "Wall thickness (mm)", "Roughness (mm)"
    };
    ...
    QStringList headerGasLift = {
        "Gas lift", "Active", "MD", "Injection basis", "Inj. quantity", "Injection unit", "Port size"
    };
    ...
    QStringList header3D = {
        "MD (m)", "TVD (m)", "Horizontal displacement (m)", "Angle (deg)", "Azimuth (deg)",
        "Max. dogleg severity  (deg/m)"
    };
    ...
    QStringList headerHTcCalATIMul = {
        "MD (m)", "Ambient temperature (degC)", "Ground density (kg/m3)", "Ground K (W/(m.K))", "Ground Cp (J/(kg.K))"
    };
...
void MWellWindow::initConnect() {
    ...
    //deviation connect
    connect(ui->surveyType, QOverload<int>::of(&QComboBox::currentIndexChanged), ui->surveyStack,
